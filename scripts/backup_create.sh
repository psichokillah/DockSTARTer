...
backup_create() {
    ...
    if [ ! -d "${SNAPSHOT_SRC}" ]; then
        warning "${SNAPSHOT_SRC} folder not found. No backup will be created."
        return
    fi
        ...
        for i in $(seq -f'%03g' "${b}" -1 "${a}"); do
            if [ -d "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.${i}" ]; then
                if [ "${f}" -eq 0 ]; then
                    ...
                    if [ "${CHATTR}" -eq 1 ]; then
                        chattr -R -i "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.${i}" > /dev/null 2>&1 || fatal "Unable to remove immutable flag from ${SNAPSHOT_DST}/${SNAPSHOT_NAME}.${i}"
                    fi
                    rm -rf "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.${i}"
                fi
            fi
        done
    ...
    remove_snapshot() {
        ...
        for i in $(seq -f'%03g' 512 -1 001); do
            if [ -d "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.${i}" ] || [ "${i}" -eq 1 ]; then
                [ ! -h "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last" ] && [ -d "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.${i}" ] && ln -s "${SNAPSHOT_NAME}.${i}" "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last"
                ...
                if [ ${d} -ne 0 ]; then #we need to remove snapshots
                    if [ "${i}" -ne 1 ]; then
                        ...
                        if [ -h "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last" ]; then
                            rm -f "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last"
                        fi
                        ...
                        if [ ${d} -eq 1 ]; then #snapshot.001 causes that free space is too small
                            if [ "${OVERWRITE_LAST}" -eq 1 ]; then #last chance: remove snapshot.001 and retry once
                                ...
                                rm -rf "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.001"
                                if [ -h "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last" ]; then
                                    rm -f "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last"
                                fi
                            else
                                ...
                                [ ! -h "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last" ] && ln -s "${SNAPSHOT_NAME}.${j}" "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last"
    ...
    while :; do #this loop is executed a 2nd time if OVERWRITE_LAST was ==1 and snapshot.001 got removed
        ...
        mkdir -p "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.test-free-disk-space"
        chmod -R 775 "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.test-free-disk-space"
        LOG="$(mktemp)"
        ...
        for i in ${LNKDST//--link-dest=/}; do
            if [ -d "${i}" ] && [ "${CHATTR}" -eq 1 ] && [ "$(lsattr -d "${i}" | cut -b5)" == "i" ]; then
                chattr -R -i "${i}" > /dev/null 2>&1 || fatal "Unable to remove immutable flag from ${i}" #unprotect last snapshots to use hardlinks
            fi
        done
        eval rsync \
            --dry-run \
            "${OPTION}" \
            "${LNKDST}" \
            "${SNAPSHOT_SRC}" "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.test-free-disk-space" >> "${LOG}" || fatal "Snapshot failed."

        i=$(($(tail -100 "${LOG}" | grep 'Total transferred file size:' | cut -d " " -f5 | sed -e 's/\,//g')/1048576))
        info "${i} MiB needed."
        rm -rf "${LOG}" "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.test-free-disk-space"
    ...
    # rotate the backups
    if [ -d "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.512" ]; then #remove snapshot.512
        info "Removing ${SNAPSHOT_DST}/${SNAPSHOT_NAME}.512 ..."
        if [ "${CHATTR}" -eq 1 ]; then
            chattr -R -i "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.512" > /dev/null 2>&1 || fatal "Unable to remove immutable flag from ${SNAPSHOT_DST}/${SNAPSHOT_NAME}.512"
        fi
        rm -rf "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.512"
    fi
    if [ -h "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last" ]; then
        rm -f "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last"
    fi
    ...
    for i in $(seq -f'%03g' 511 -1 000); do
        if [ -d "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.${i}" ]; then
            ...
            if [ "${CHATTR}" -eq 1 ]; then
                chattr -i "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.${i}" > /dev/null 2>&1 || fatal "Unable to remove immutable flag from ${SNAPSHOT_DST}/${SNAPSHOT_NAME}.${i}"
            fi
            ...
            if [ ! -h "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last" ]; then
                ln -s "${SNAPSHOT_NAME}.${j}" "${SNAPSHOT_DST}/${SNAPSHOT_NAME}.last"
            fi
        fi
    done
